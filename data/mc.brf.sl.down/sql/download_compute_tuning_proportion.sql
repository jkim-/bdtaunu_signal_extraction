...
FROM 
  ...
  -- Isolate sample type and attach evttype
  (SELECT eid, grouped_dss_evttype AS evttype
    FROM 
      (sample_assignments_generic 
       INNER JOIN
       event_labels_generic_augmented
       USING (eid))
    WHERE sample_type=5) AS Q1
...
\copy (SELECT * FROM test_sample) TO 'compute_truth.tuning.csv' WITH DELIMITER ' ';
